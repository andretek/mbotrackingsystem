...
if (isset($_POST["submit"])) {
  ...
  $sql = "UPDATE `accountspayable` SET `office_code`='$OFFICECODE',`name_office`='$OFFICENAME',`payee`='$PAYEE',`discription`='$DISCRIPTION',`account_code`='$ACCOUNTCODE',`obr`='$OBR', `amount`='$AMOUNT' WHERE id = $id";

  $result = mysqli_query($conn, $sql);

  if ($result) {
    header("Location: apindex.php?msg=Data updated successfully");
  } else {
...
<body>
  ...
  <div class="container">
    <div class="text-center mb-4">
      <h3>Accounts Payable ENTRY FORM</h3>
      <p class="text-muted">Click update after changing any information</p>
    </div>

    <?php
    $sql = "SELECT * FROM `accountspayable` WHERE id = $id LIMIT 1";
    $result = mysqli_query($conn, $sql);
